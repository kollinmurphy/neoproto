...
async function run({
  ...
  console.log(
    `Found API namespace: ${apiNamespace.fullName} (Base Name: ${baseName}, Version: ${version})`,
  );
  let topLevelMessages = getMessages(apiNamespace).filter(isTopLevelMessage);
  if (topLevelMessages.length === 0) {
    topLevelMessages = getMessages(apiNamespace);
  }
  ...
  await writeFile(
    `${outDir}/wrap.ts`,
    prependHeader(createRootNamespaceWrappers(apiNamespace)),
  );
  ...
  await writeFile(
    `${outDir}/types.ts`,
    prependHeader(createRootNamespaceTypes(apiNamespace)),
  );
